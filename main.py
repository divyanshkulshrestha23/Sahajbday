...
app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get('DB_URI', "sqlite:///memories1.db" )
db.init_app(app)
...
@app.route("/")
def home():
    memories = db.session.execute(db.select(Memories1).order_by(Memories1.rating.asc()))
...
def update():
    ...
    memorySelected = db.get_or_404(Memories1, memoryID)
...
def delete():
    memoryID = request.args.get('id')
    memoryDeleted = db.get_or_404(Memories1, memoryID)
